...
export function ArrIntegrationForm({ provider, locale, overview, rootFolders, qualityProfiles, tags }: { provider: "radarr" | "sonarr"; locale: string; overview: { baseUrl: string; hasApiKey: boolean; encryptionConfigured: boolean; rootFolderPath?: string; qualityProfileId?: number; tagIds: number[]; searchOnAdd: boolean; seriesMonitor: string }; rootFolders: Option[]; qualityProfiles: Option[]; tags: Option[] }) {
  ...
  const [baseUrl, setBaseUrl] = useState(overview.baseUrl); const [apiKey, setApiKey] = useState("");
  ...
  return <form action={action} onReset={(event) => event.preventDefault()} className="space-y-5">
    ...
    <div className="space-y-2"><Label htmlFor={`${provider}-url`}>{t("baseUrl")}</Label><Input id={`${provider}-url`} name="baseUrl" type="url" required value={baseUrl} onChange={(event) => setBaseUrl(event.target.value)} placeholder={provider === "radarr" ? "http://radarr:7878" : "http://sonarr:8989"} /><p className="text-xs text-muted-foreground">{t("baseUrlHelp")}</p></div>
    <div className="space-y-2"><Label htmlFor={`${provider}-key`}>{t("apiKey")}</Label><Input id={`${provider}-key`} name="apiKey" type="password" value={apiKey} onChange={(event) => setApiKey(event.target.value)} placeholder={overview.hasApiKey ? t("keyStored") : undefined} disabled={!overview.encryptionConfigured} autoComplete="off" /><p className="text-xs text-muted-foreground">{overview.encryptionConfigured ? t("apiKeyHelp") : t("encryptionHelp")}</p></div>
