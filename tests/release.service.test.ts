...
describe("ReleaseService", () => {
  ...
  it("caches a GitHub release and exposes its notes", async () => {
    ...
    const repository = { getCachedRelease: vi.fn(() => Promise.resolve(cached)), saveCachedRelease: vi.fn(async (payload) => { cached = { payload, updatedAt: new Date() }; }) };
    const request = vi.fn().mockResolvedValue({ ok: true, json: async () => ({ tag_name: "v9.0.0", html_url: "https://example.test/release", body: "Important fixes", published_at: "2026-08-30T00:00:00Z", prerelease: false }) });
