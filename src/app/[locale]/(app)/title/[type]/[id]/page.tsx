...
export default async function TitlePage({ params }: { params: Promise<{ type: string; id: string }> }) {
  ...
  return <div className="space-y-10">
    ...
    {trailer && <section><h2 className="font-display text-3xl font-semibold tracking-tight">{t("trailer")}</h2><div className="mt-5 aspect-video w-full overflow-hidden rounded-3xl border border-border bg-black"><iframe src={`https://www.youtube-nocookie.com/embed/${encodeURIComponent(trailer.key)}`} title={trailer.name} className="size-full" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen /></div></section>}
