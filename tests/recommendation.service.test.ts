...
describe("RecommendationService", () => {
  it("allows a non-personalized refresh before taste signals exist", async () => {
    const repository = {
      ...
      getLatestRun: vi.fn().mockResolvedValue({
        id: "failed-run",
        status: "failed",
        error: "No positive taste signals were found for recommendation discovery",
      }),
  ...
  it("keeps current recommendations during the configured quiet period", async () => {
    const repository = {
      ...
      getRefreshState: vi.fn().mockResolvedValue({
        ...
        refreshAfter: new Date(Date.now() + 60_000),
      }),
    } as unknown as RecommendationRepository;
  ...
  it("reserves similarity seeds for rated series when movies are more numerous", () => {
    const signals: RecommendationSignal[] = [
      ...Array.from({ length: 10 }, (_, index): RecommendationSignal => ({
        tmdbId: index + 1,
        type: "movie",
        rating: 5,
        tags: [],
  ...
  it("generates separate persisted movie and series candidates from private taste signals", async () => {
    const repository = {
      getSignals: vi
        .fn()
        .mockResolvedValue([
          { tmdbId: 1, type: "movie", played: true, playedPercentage: 100, rating: 5, excluded: false },
        ]),
      getPositiveFeedback: vi.fn().mockResolvedValue([]),
      getRefreshState: vi.fn().mockResolvedValue(undefined),
      getExistingScores: vi.fn().mockResolvedValue(new Map()),
    ...
    expect(repository.saveRecommendations).toHaveBeenCalledWith(
      "user",
      expect.arrayContaining([expect.objectContaining({ id: 20, type: "series" })]),
    );
    expect(repository.saveRecommendations).not.toHaveBeenCalledWith(
      "user",
      expect.arrayContaining([expect.objectContaining({ id: 10, type: "movie" })]),
    );
  ...
  it("keeps recommendations stable while signals and locale remain unchanged", async () => {
    ...
    const repository = {
      ...
      getWatchedTitles: vi.fn().mockResolvedValue([]),
      saveRecommendations: vi.fn(),
      setRefreshState: vi.fn((_userId: string, signalFingerprint: string, locale: string) => {
        state = { signalFingerprint, locale, refreshedAt: new Date() };
      }),
    } as unknown as RecommendationRepository;
    const metadata = {
      getTitleMetadata: vi.fn().mockResolvedValue({ genres: [{ id: 28, name: "Action" }] }),
