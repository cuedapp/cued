...
describe("OpenAiClient", () => {
  it("uses the Responses API with encrypted-key-compatible bearer authentication and strict structured output", async () => {
    ...
    const result = await new OpenAiClient(transport).generateTasteProfile("secret-key", "gpt-5-mini", "en", [
      { title: "Example", type: "movie", rating: 5, tags: ["smart"], watched: true },
    ]);
  ...
  it("returns sanitized provider errors", async () => {
    ...
    await expect(new OpenAiClient(transport).testConnection("bad-key", "gpt-5-mini")).rejects.toEqual(
      expect.objectContaining<OpenAiRequestError>({
  ...
  it("returns actionable validation errors without leaking the API key", async () => {
    const transport = vi.fn<typeof fetch>().mockResolvedValue(
      response(
        {
          error: {
            message: "Unsupported parameter for secret-key",
    ...
    await expect(new OpenAiClient(transport).testConnection("secret-key", "gpt-5.6-luna")).rejects.toEqual(
      expect.objectContaining<OpenAiRequestError>({
        name: "OpenAiRequestError",
        status: 400,
