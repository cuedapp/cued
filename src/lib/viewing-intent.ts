export const viewingIntentPresets = [
  "easyWatch",
  "action",
  "clever",
  "lightFunny",
...
const presetGenres: Partial<Record<ViewingIntentPreset, number[]>> = {
  easyWatch: [35, 10751, 16, 10749],
  action: [28, 12, 53, 80, 10752, 37],
  clever: [9648, 878, 99, 36],
  lightFunny: [35, 10751, 16],
  comfort: [35, 10751, 16, 10749],
  suspense: [53, 9648, 80],
  romance: [10749],
  fantasy: [14, 878, 12],
  scary: [27],
  documentary: [99, 36, 10402],
  animation: [16],
  family: [10751, 16],
};
  ...
  const moodMatch =
    ...
    moodPresets.some(
      (preset) =>
        ...
        viewingIntentPresetTerms[preset as keyof typeof viewingIntentPresetTerms].some((term) =>
...
function intentBoost(item: IntentRecommendation, presets: ViewingIntentPreset[], terms: string[]) {
  ...
  const genreMatches = presets.reduce(
    (total, preset) => total + (presetGenres[preset]?.some((genre) => item.genreIds.includes(genre)) ? 1 : 0),
    0,
  );
  const typeBoost =
    (presets.includes("movieTonight") && item.mediaType === "movie" ? 12 : 0) +
