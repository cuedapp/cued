...
export function RequestPolicyForm({
  ...
  useEffect(() => {
    if (state.result) toast.success(t("requestPolicySaved"));
    if (state.error) toast.error(t("requestPolicyFailed"));
  }, [state, t]);
  ...
  return (
    ...
    >
      ...
      <FormSubmitButton size="sm" pendingLabel={t("savingRequestPolicy")}>
        {t("saveRequestPolicy")}
      </FormSubmitButton>
    </form>
  );
