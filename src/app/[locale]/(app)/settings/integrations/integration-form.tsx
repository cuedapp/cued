...
export function IntegrationForm({ locale, baseUrl, encryptionConfigured, hasApiKey }: { locale: string; baseUrl: string; encryptionConfigured: boolean; hasApiKey: boolean }) {
  ...
  const [url, setUrl] = useState(baseUrl); const [apiKey, setApiKey] = useState("");
  useEffect(() => {
  ...
  return <form action={action} onReset={(event) => event.preventDefault()} className="space-y-5">
    ...
    <div className="space-y-2"><Label htmlFor="baseUrl">{t("url")}</Label><Input id="baseUrl" name="baseUrl" type="url" value={url} onChange={(event) => setUrl(event.target.value)} required autoComplete="url" /></div>
    <div className="space-y-2"><Label htmlFor="apiKey">{t("apiKey")}</Label><Input id="apiKey" name="apiKey" type="password" value={apiKey} onChange={(event) => setApiKey(event.target.value)} placeholder={hasApiKey ? t("keyStored") : undefined} disabled={!encryptionConfigured} autoComplete="off" /><p className="text-xs leading-5 text-muted-foreground">{encryptionConfigured ? t("apiKeyHelp") : t("encryptionHelp")}</p></div>
