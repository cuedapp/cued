...
describe("M3uEditorClient", () => {
  ...
  it("authenticates without putting credentials in the URL", async () => {
    const fetchMock = vi.fn().mockResolvedValue(new Response(JSON.stringify({ user_info: { auth: 1 } }), { status: 200 })); vi.stubGlobal("fetch", fetchMock);
  ...
  it("reports provider failures without response bodies", async () => { vi.stubGlobal("fetch", vi.fn().mockResolvedValue(new Response("secret upstream body", { status: 401 }))); await expect(new M3uEditorClient().authenticate(connection)).rejects.toEqual(new M3uEditorRequestError("M3U Editor returned 401")); });

  it("loads and sorts series episodes for STRM generation", async () => { const fetchMock = vi.fn().mockResolvedValue(new Response(JSON.stringify({ episodes: { "2": [{ id: 22, episode_num: 1, title: "Return", container_extension: "mp4" }], "1": [{ id: 11, episode_num: 2, title: "Next", container_extension: ".mkv" }, { id: 10, episode_num: 1, title: "Pilot" }] } }), { status: 200 })); vi.stubGlobal("fetch", fetchMock); await expect(new M3uEditorClient().getSeriesEpisodes(connection, "99")).resolves.toEqual([{ externalId: "10", seasonNumber: 1, episodeNumber: 1, title: "Pilot", containerExtension: "mkv" }, { externalId: "11", seasonNumber: 1, episodeNumber: 2, title: "Next", containerExtension: "mkv" }, { externalId: "22", seasonNumber: 2, episodeNumber: 1, title: "Return", containerExtension: "mp4" }]); expect(String(fetchMock.mock.calls[0]![1].body)).toContain("series_id=99"); });
});
