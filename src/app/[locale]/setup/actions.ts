...
export interface SetupFormState { error?: "invalid" | "unreachable" | "encryption" }
...
export async function configureJellyfin(_: SetupFormState, formData: FormData): Promise<SetupFormState> {
  const result = setupSchema.safeParse({ locale: formData.get("locale"), baseUrl: formData.get("baseUrl"), apiKey: formData.get("apiKey") });
