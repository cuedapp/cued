...
describe("MediaRatingService", () => {
  ...
  it("enriches series from TMDB and movies from TMDB plus Radarr", async () => {
    const repository = {
      ...
      listForRun: vi.fn().mockResolvedValue([
        { mediaType: "series", tmdbId: 10 },
        { mediaType: "movie", tmdbId: 20 },
      ]),
    ...
    const tmdb = {
      getTitleMetadata: vi
        ...
        .mockImplementation((type: string) =>
          Promise.resolve({ rating: type === "series" ? 8.4 : 7.1, voteCount: 900 }),
        ),
    } as unknown as TmdbMetadataService;
    const radarr = {
      lookupMetadata: vi.fn().mockResolvedValue({
        raw: { ratings: { imdb: { value: 7.5, votes: 1000 }, tmdb: { value: 7.2, votes: 800 } } },
      }),
    } as unknown as ArrIntegrationService;
    const now = new Date("2026-08-31T12:00:00.000Z");

    await expect(new MediaRatingService(repository, tmdb, radarr).enrichDue(now)).resolves.toEqual({
      checked: 2,
      enriched: 2,
      failed: 0,
      completed: true,
  ...
  it("does no work when no weekly refresh is due", async () => {
    const repository = {
      getActiveRun: vi.fn().mockResolvedValue(undefined),
      hasDue: vi.fn().mockResolvedValue(false),
    } as unknown as MediaRatingRepository;
    const service = new MediaRatingService(repository, {} as TmdbMetadataService, {} as ArrIntegrationService);

    await expect(service.enrichDue(new Date("2026-08-31T12:00:00.000Z"))).resolves.toEqual({
      checked: 0,
      enriched: 0,
      failed: 0,
      completed: false,
  ...
  it("refreshes a detail page on demand and returns ratings in display order", async () => {
    ...
    const radarr = {
      lookupMetadata: vi.fn().mockResolvedValue({
        raw: { ratings: { imdb: { value: 7.5, votes: 1000 }, rottenTomatoes: { value: 91, votes: 50 } } },
      }),
    } as unknown as ArrIntegrationService;
    const now = new Date("2026-08-31T12:00:00.000Z");

    const result = await new MediaRatingService(repository, {} as TmdbMetadataService, radarr).getTitleRatings(
      "movie",
      20,
      7.2,
      800,
