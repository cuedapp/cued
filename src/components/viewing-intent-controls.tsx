...
export function ViewingIntentControls({ presets, text, onPresetsChange, onTextChange }: { presets: ViewingIntentPreset[]; text: string; onPresetsChange: (presets: ViewingIntentPreset[]) => void; onTextChange: (text: string) => void }) {
  ...
  return <>
    <section className="border-y border-border bg-muted/20 py-4 sm:rounded-lg sm:border sm:px-5">
      ...
      <label className="mt-3 block"><span className="sr-only">{t("freeText")}</span><input value={text} onChange={(event) => onTextChange(event.target.value)} maxLength={120} placeholder={t("placeholder")} className="h-10 w-full rounded-lg border border-input bg-background px-3 text-sm" /></label>
    </section>
