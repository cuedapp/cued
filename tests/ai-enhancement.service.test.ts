...
const candidate: ScoredRecommendation = {
  id: 10,
  type: "movie",
  title: "Candidate",
  overview: "Story",
...
describe("AiEnhancementService", () => {
  it("leaves deterministic recommendations untouched when AI is disabled", async () => {
    ...
    await expect(
      new AiEnhancementService({} as AiRepository, integration).enhance("user", "en", signals, [candidate]),
  ...
  it("caches profile-aware reranking and blends it without replacing deterministic scoring", async () => {
    const provider = {
      ...
      generateTasteProfile: vi.fn().mockResolvedValue({ summary: "Profile", traits: ["Drama"], dislikes: [] }),
      rerank: vi
        .fn()
        .mockResolvedValue([
          { id: 10, type: "movie", score: 90, explanation: "Matches your preference for focused drama." },
        ]),
    };
    const integration = {
      getConnection: vi
        .fn()
        .mockResolvedValue({ providerId: "openai", apiKey: "key", model: "gpt-5-mini", mode: "balanced" }),
      execute: (operation: (provider: AiProvider, apiKey: string, model: string) => Promise<unknown>) =>
        operation(provider as unknown as AiProvider, "key", "gpt-5-mini"),
    } as unknown as AiIntegrationService;
    let cached:
      { recommendations: Array<{ id: number; type: "movie"; score: number; explanation: string }> } | undefined;
    const repository = {
      getProfile: vi.fn().mockResolvedValue(undefined),
      saveProfile: vi.fn(),
      getCached: vi.fn(() => Promise.resolve(cached)),
      setCached: vi.fn((_provider, _key, value) => {
        cached = value;
      }),
  ...
  it("normalizes fractional AI scores to the documented 0-100 scale", async () => {
    const provider = {
      generateTasteProfile: vi.fn().mockResolvedValue({ summary: "Profile", traits: [], dislikes: [] }),
      rerank: vi.fn().mockResolvedValue([{ id: 10, type: "movie", score: 0.88, explanation: "Strong match." }]),
    };
    const integration = {
      getConnection: vi
        .fn()
        .mockResolvedValue({ providerId: "openai", apiKey: "key", model: "gpt-5.6-luna", mode: "balanced" }),
      execute: (operation: (provider: AiProvider, apiKey: string, model: string) => Promise<unknown>) =>
        operation(provider as unknown as AiProvider, "key", "gpt-5.6-luna"),
    } as unknown as AiIntegrationService;
    const repository = {
      getProfile: vi.fn().mockResolvedValue(undefined),
      saveProfile: vi.fn(),
    ...
    const [result] = await new AiEnhancementService(repository, integration).enhance("user", "en", signals, [
      candidate,
    ]);
  ...
  it("reserves AI shortlist capacity for both movies and series", async () => {
    const candidates = Array.from({ length: 12 }, (_, index): ScoredRecommendation => ({
      ...candidate,
      id: index + 1,
      type: index < 10 ? "movie" : "series",
    }));
    const provider = {
      generateTasteProfile: vi.fn().mockResolvedValue({ summary: "Profile", traits: [], dislikes: [] }),
      rerank: vi.fn().mockImplementation((_key, _model, _locale, _profile, shortlist) =>
        Promise.resolve(
          shortlist.map((item: { id: number; type: "movie" | "series" }) => ({
            ...item,
            score: 80,
            explanation: "Match.",
          })),
