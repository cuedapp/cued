...
export function NtfyForm({ locale, overview }: { locale: string; overview: Awaited<ReturnType<typeof import("@/server/application/notification.service").NotificationService.prototype.getNtfyOverview>> }) {
    ...
    <form action={action} className="flex flex-1 flex-col">
      ...
      <div className="space-y-5 px-6 pb-6">
        ...
        <div className="space-y-2">
          ...
          <Input id="ntfy-topic" name="topic" defaultValue={overview.topic} placeholder={t("topicPlaceholder")} required />
          <p className="text-xs text-muted-foreground">{t("topicHelp")}</p>
        </div>
        <div className="space-y-2">
          <Label htmlFor="ntfy-token">{t("token")}</Label>
          <Input id="ntfy-token" name="token" type="password" placeholder={overview.hasToken ? "••••••••" : t("tokenOptional")} disabled={!overview.encryptionConfigured} autoComplete="off" />
          <p className="text-xs text-muted-foreground">{overview.encryptionConfigured ? t("tokenHelp") : t("errors.encryption")}</p>
        </div>
        <div className="rounded-xl border border-border">
          ...
          >
            <Label htmlFor="ntfy-threshold" className={!integrationFailuresEnabled ? "text-muted-foreground" : undefined}>
              {t("failureThreshold")}
            </Label>
        ...
        <label className="flex cursor-pointer items-start gap-3 rounded-xl border border-border p-4">
          <input name="jobFailures" type="checkbox" defaultChecked={overview.jobFailures} className="mt-1 size-4 accent-primary" />
          <span>
        ...
        <label className="flex cursor-pointer items-start gap-3 rounded-xl border border-border p-4">
          <input name="updates" type="checkbox" defaultChecked={overview.updates} className="mt-1 size-4 accent-primary" />
          <span>
