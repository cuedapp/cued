...
export class OperationalRepository {
  async getCachedRelease() {
    return db.query.metadataCacheEntries.findFirst({
      where: (entry, { and, eq, gt }) =>
        and(
          eq(entry.provider, "github"),
          eq(entry.cacheKey, "latest-release"),
          eq(entry.locale, "neutral"),
          gt(entry.expiresAt, new Date()),
        ),
    });
  }
  ...
  async clearCaches() {
    return (await db.delete(metadataCacheEntries).returning({ id: metadataCacheEntries.id })).length;
  }
  async cacheCount() {
    const [result] = await db.select({ value: count() }).from(metadataCacheEntries);
    return Number(result?.value ?? 0);
  }
  async integrationDiagnostics() {
    return db
      .select({
