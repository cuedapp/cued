...
export function OpenAiIntegrationForm({
  ...
  configurations: Record<AiProviderId, ProviderConfiguration>;
}) {
  ...
  useEffect(() => {
    if (state.error)
      toast.error(
        state.error.startsWith("openrouter") ? connectionT(`errors.${state.error}`) : t(`errors.${state.error}`),
      );
    if (state.result)
      toast.success(
        state.result.startsWith("openrouter") ? connectionT(`results.${state.result}`) : t(`results.${state.result}`),
      );
  }, [connectionT, state, t]);
  function selectProvider(nextProvider: AiProviderId) {
    const next = configurations[nextProvider];
    const models = nextProvider === "openrouter" ? openRouterModels : openAiModels;
    const known = models.some((item) => item.id === next.model);
    setProvider(nextProvider);
    setSelectedModel(known ? next.model : customOption);
    setCustomModel(known ? "" : next.model);
    setSelectedMode(next.mode);
    setRefreshDelayMinutes(next.refreshDelayMinutes);
    setApiKey("");
  }
  ...
  return (
    <form action={action} onReset={(event) => event.preventDefault()} className="space-y-5">
      ...
      <div className="space-y-2">
        <Label htmlFor="aiProvider">{providerT("provider")}</Label>
        <select
          id="aiProvider"
          value={provider}
          onChange={(event) => selectProvider(event.target.value as AiProviderId)}
          className="h-10 w-full cursor-pointer rounded-lg border border-input bg-background px-3 text-sm"
        >
          <option value="openai">{providerT("providers.openai")}</option>
          <option value="openrouter">{providerT("providers.openrouter")}</option>
        </select>
        <p className="text-xs leading-5 text-muted-foreground">{providerT(`providerHelp.${provider}`)}</p>
      </div>
      <div className="space-y-2">
        ...
        <select
          ...
          name="mode"
          value={selectedMode}
          onChange={(event) => setSelectedMode(event.target.value as AiMode)}
          className="h-10 w-full cursor-pointer rounded-lg border border-input bg-background px-3 text-sm"
        >
          <option value="off">{t("modeLabels.off")}</option>
          <option value="conservative">{t("modeLabels.conservative")}</option>
          <option value="balanced">{t("modeLabels.balanced")}</option>
          <option value="enhanced">{t("modeLabels.enhanced")}</option>
        </select>
      ...
      <div className="space-y-2">
        ...
        <select
          id="refreshDelayMinutes"
          name="refreshDelayMinutes"
          value={refreshDelayMinutes}
          onChange={(event) => setRefreshDelayMinutes(Number(event.target.value))}
          className="h-10 w-full cursor-pointer rounded-lg border border-input bg-background px-3 text-sm"
        >
          {[0, 5, 15, 30].map((minutes) => (
            <option key={minutes} value={minutes}>
              {refreshDelayT(`options.${minutes}`)}
            </option>
          ))}
        </select>
      ...
      <div className="space-y-2">
        ...
        >
          {availableModels.map((candidate) => (
            <option key={candidate.id} value={candidate.id}>
              {provider === "openrouter"
                ? openRouterModelT(`${candidate.translationKey}.label`)
                : t(`models.${candidate.translationKey}.label`)}
            </option>
          ))}
          <option value={customOption}>{t("models.custom.label")}</option>
        </select>
        {selectedModel === customOption ? (
          <div className="space-y-2 pt-1">
            <Label htmlFor="openAiCustomModel">{t("customModel")}</Label>
            <Input
        ...
        <p className="text-xs leading-5 text-muted-foreground">
          {selectedModel === customOption
            ...
            : provider === "openrouter"
              ? openRouterModelT(`${option!.translationKey}.description`)
              : t(`models.${option!.translationKey}.description`)}
        </p>
        {option ? (
          <div className="rounded-xl border border-border bg-muted/40 p-4">
            ...
            <dl className="mt-3 grid grid-cols-3 gap-3 text-sm">
              <div>
                <dt className="text-muted-foreground">{t("costDay")}</dt>
                <dd className="mt-1 font-semibold">{formatCost(estimatedCost(option.input, option.output))}</dd>
              </div>
              <div>
                <dt className="text-muted-foreground">{t("costWeek")}</dt>
                <dd className="mt-1 font-semibold">{formatCost(estimatedCost(option.input, option.output) * 7)}</dd>
              </div>
              <div>
                <dt className="text-muted-foreground">{t("costMonth")}</dt>
                <dd className="mt-1 font-semibold">{formatCost(estimatedCost(option.input, option.output) * 30)}</dd>
              </div>
            </dl>
