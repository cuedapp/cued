...
Application-level visibility settings are stored separately from integration configuration. Administrators can expose aggregate server statistics to regular users without exposing the administrator-only per-user breakdown. A separate recent-activity permission enables a dashboard feed with user identity; its query still intersects every activity item with the signed-in viewer's Jellyfin library access, so enabling the feature does not disclose titles from inaccessible libraries. Both permissions default to off.
...
Candidates are stored per user with their localized metadata, calculated match percentage, TMDB rating, vote count and popularity, title-specific sources, genre reasons and feedback state. Refreshes upsert candidates without deleting older discoveries or replacing their original added timestamp, turning the full Recommendations page into a persistent, filterable and sortable inbox; an explicit confirmed “start fresh” action clears that user’s inbox before regenerating it. Availability is resolved against the user's current Jellyfin library permissions when recommendations are read. Users can request more similar suggestions, hide a recommendation and restore hidden items. The dashboard rotates its stored shortlist with a deterministic per-user daily order, and its taste-building prompt remains visible until at least five usable viewing or rating signals exist. Dashboard access refreshes immediately when its signal fingerprint or locale changes and otherwise after 24 hours. A guarded in-process scheduler checks hourly for profiles whose daily refresh is due, matching Cued's single-container deployment model. Manual refresh is also available.

## Optional AI enhancement

OpenAI and OpenRouter are implemented behind one AI provider contract and configured in the application with separate encrypted API keys, curated model presets and a shared usage mode. OpenRouter requests require zero-data-retention routing, deny data collection and require strict structured-output support. Off leaves the deterministic pipeline unchanged. Conservative, Balanced and Enhanced send progressively larger local shortlists of 8, 12 or 20 candidates and blend AI reranking at 15%, 25% or 35%; AI never supplies the candidate universe. Private taste signals—including ratings, tags, watch state and optional written feedback—produce a concise per-user profile. Signal fingerprints prevent unchanged profiles from being regenerated, while profile-and-shortlist fingerprints cache reranking for 30 days. A configurable 0, 5, 15 or 30-minute quiet period batches repeated taste changes; manual refreshes bypass it. Administrators can separately enable the conversational recommendation assistant per user and set a 1–100 request daily limit. Each request is atomically claimed against that limit and can return only titles from the user's age-filtered, private recommendation catalogue; provider-invented identifiers are discarded. Successful questions, answers and grounded recommendation snapshots are stored privately per user so recent conversations can be reopened; they cascade when the user is removed and are included in full backups. Provider responses record cumulative input tokens, output tokens and actual or preset-derived cost across profile generation, reranking and conversational requests. Provider-independent fixtures verify that every implementation satisfies the same profile and reranking contract. Provider errors fall back to deterministic results rather than failing recommendation generation. Users can explicitly refresh their AI profile.
...
Cued also keeps a private in-app notification inbox. Recommendation work is visible only to the affected user, while integration work is visible only to administrators. Rating enrichment notifies administrators once the complete multi-batch synchronization finishes and when the scheduled synchronization fails; intermediate and empty batches remain silent. The authenticated shell uses one visibility-aware status coordinator to poll jobs, recommendation refresh state and unread events without overlapping requests. It distributes that state to the job indicator, notification toasts and unread navigation count, while page-specific data such as live playback remains independently scoped. Users can clear only their own history.
...
Pages and application data are server-rendered by default. Client Components are limited to browser-dependent theme state, interactive forms, account menus, status coordination and toast feedback. URL-backed filter forms use the shared client-navigation helper so applying filters refreshes Server Component data without a native document reload. The desktop sidebar is viewport-bound with an independently scrollable navigation region and persistent account controls; mobile navigation uses a compact menu popover. Persistent primary navigation deliberately disables route prefetching, avoiding a burst of data-heavy dynamic route work while leaving normal client-side navigation intact. Search, title and person pages are server rendered. Theme preference is persisted in a cookie for immediate server rendering; language is persisted on the user and restored at login; the dashboard greeting uses browser-local time. Media artwork uses a loading treatment and a type-specific fallback, detail pages provide browser-history back navigation, and person credits can be filtered and sorted. All user-facing messages are maintained in English, Swedish and Dutch. The About card includes TMDB's required attribution and approved logo.
...
Every user has a private profile containing their completed-viewing summary, recent trend, viewing insights and paginated acquisition-request history. A regular user can resolve only their own profile; administrators reach another user's profile through an explicit link in Users administration. Request rows remain keyed to TMDB titles independently of current library presence. Approved provider requests are presented as requested until an active accessible Jellyfin match exists, at which point the profile derives a completed/available presentation without changing the durable approval audit state.

## Viewing intent

Ask Cued offers a private recommendation-catalogue scope and a broader TMDB discovery scope. Broader discovery candidates are filtered before they reach the AI provider: titles are excluded when they are already in the user's accessible Jellyfin library, already present in Cued's recommendation catalogue, watched or partially watched, outside the user's preferred original languages, or restricted by the user's content-guidance setting. Saved conversations retain the grounded recommendation snapshot that was shown at the time.
...
The dashboard includes a read-only activity view: the signed-in user's recently completed titles, estimated watch time from the runtime of completed movies and episodes (each counted once), and a Monday–Sunday personal activity trend. Activity statistics count a completion only when Jellyfin supplies both a played state and a last-played timestamp; undated historical flags remain usable as recommendation signals but are not presented as viewing activity. Popular titles and aggregate ratings are server-wide only at the title level and include titles in selected Jellyfin libraries the signed-in user can access; no user-level viewing or rating activity is exposed. Each user profile shows that user's private completed-watch and rating statistics. Administrators additionally have a Statistics page with selected-library movie/series totals, aggregate watch activity, most-watched titles, aggregate rating extremes, interactive viewing insights, and per-user summaries. Administrators can disable Cued access independently of Jellyfin, which revokes existing sessions and excludes that user from shared statistics, and set the application-wide user display order; they cannot disable their own account. Monthly and yearly recaps are not implemented.

## Backup and portability

The Settings page exposes a versioned JSON user export to every authenticated user. It contains only portable, user-owned preferences: display format, preferred discovery languages, AI taste profile, ratings, written feedback, exclusions and follows. Import applies preferences and follows to the signed-in user only. Feedback is matched to current local media by its TMDB ID and media type, so records for titles not yet present in the receiving Jellyfin library are reported as skipped rather than being attached to another title.

Administrators download full versioned database archives as gzip-compressed JSON (`.json.gz`). Full restore uses the protected backup route rather than a Server Action, accepts the new compressed format and legacy plain JSON, and limits uploads and decompressed payloads to 100 MB. Full archives include durable application and integration configuration records, including encrypted credential ciphertext, conversational-AI quota counters and saved conversation history, but exclude sign-in sessions and recreatable metadata, availability and search caches. Restoring a full archive truncates and replaces the application data in one database transaction, so it requires an explicit browser confirmation and all users must sign in again. Encrypted credentials remain usable only when the restored installation has the same `CUED_ENCRYPTION_KEY`.
