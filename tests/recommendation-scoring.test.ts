...
import { buildGenreTaste, scoreCandidates, type RecommendationSignal } from "@/server/application/recommendation-scoring";

const signals: RecommendationSignal[] = [
  { tmdbId: 1, type: "movie", rating: 5, played: true, playedPercentage: 100, excluded: false, genres: [{ id: 28, name: "Action" }] },
  { tmdbId: 2, type: "movie", rating: 1, played: true, playedPercentage: 100, excluded: false, genres: [{ id: 27, name: "Horror" }] },
];
...
describe("recommendation scoring", () => {
  ...
  it("filters watched titles and prioritizes candidates matching positive genres", () => {
    const candidates = [
      { id: 1, type: "movie" as const, title: "Seen", overview: "", genreIds: [28], rating: 9, voteCount: 1000, popularity: 100 },
      { id: 10, type: "movie" as const, title: "Action candidate", overview: "", genreIds: [28], rating: 7, voteCount: 500, popularity: 20 },
      { id: 11, type: "movie" as const, title: "Horror candidate", overview: "", genreIds: [27], rating: 8, voteCount: 500, popularity: 20 },
    ];
  ...
  it("filters watched titles outside the limited taste-signal set", () => {
    const candidates = [{ id: 10, type: "movie" as const, title: "Previously watched", overview: "", genreIds: [28], rating: 8, voteCount: 500, popularity: 20 }];
  ...
  it("boosts candidates recommended from a strongly liked title", () => {
    const candidates = [
      { id: 10, type: "movie" as const, title: "Direct match", overview: "", genreIds: [28], rating: 7, voteCount: 500, popularity: 20 },
      { id: 11, type: "movie" as const, title: "Genre match", overview: "", genreIds: [28], rating: 7, voteCount: 500, popularity: 20 },
    ];
    const scored = scoreCandidates(candidates, signals, new Map(), new Set(), new Map([["movie:10", [{ id: 1, type: "movie", title: "Seed", reason: "liked" }]]]));
