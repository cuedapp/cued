...
export class AiIntegrationService {
  ...
  constructor(
    ...
    providers: AiProvider | Record<AiProviderId, AiProvider> = new OpenAiClient(),
  ) {
  ...
  async configure(input: {
    ...
    if (!this.encryption && (input.apiKey || input.mode !== "off"))
      throw new Error("Encryption must be configured before enabling AI");
    ...
    if (input.mode !== "off")
      await this.providers[providerId].testConnection(
        normalizedKey || this.encryption!.decrypt(encryptedApiKey!),
        model,
      );
    return this.repository.saveIntegration(
      providerId,
      encryptedApiKey,
      input.mode,
      model,
      input.refreshDelayMinutes ?? defaultRefreshDelayMinutes,
    );
  }
  ...
  async testConfiguration(input: { provider?: AiProviderId; apiKey?: string; model?: string }) {
    ...
    const apiKey =
      input.apiKey?.trim() ||
      (integration?.encryptedApiKey && this.encryption
  ...
  async getRefreshDelayMinutes() {
    ...
    return configured === 0 || configured === 5 || configured === 15 || configured === 30
      ? configured
      : defaultRefreshDelayMinutes;
  }
