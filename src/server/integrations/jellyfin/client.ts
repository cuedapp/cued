...
export function normalizeJellyfinUrl(value: string): string {
  ...
  if (url.username || url.password || url.search || url.hash)
    throw new Error("Jellyfin URL cannot contain credentials, a query, or a fragment");
...
export class JellyfinClient implements MediaServerProvider {
  ...
  async authenticate(username: string, password: string): Promise<MediaServerAuthentication> {
    const result = authenticationSchema.parse(
      await this.request("/Users/AuthenticateByName", {
        method: "POST",
  ...
  async getUsers(apiKey: string): Promise<MediaServerUser[]> {
    return z
      .array(userSchema)
      .parse(await this.request("/Users", { apiKey }))
      .map(mapUser);
  }
    ...
    const userData = item.UserData
      ? {
          ...
          ...(item.UserData.PlaybackPositionTicks !== undefined
            ? { playbackPositionTicks: String(item.UserData.PlaybackPositionTicks) }
            : {}),
  ...
  private async request(
    path: string,
    options: { method?: "GET" | "POST"; apiKey?: string; body?: Record<string, unknown> } = {},
  ): Promise<unknown> {
  ...
  private async requestImage(path: string, apiKey: string): Promise<MediaServerImage | undefined> {
    ...
    try {
      ...
      throw new JellyfinRequestError(0, "Jellyfin could not be reached");
    }
    if (response.status === 404) return undefined;
    if (!response.ok) throw new JellyfinRequestError(response.status);
    return {
