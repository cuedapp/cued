...
export function MediaPoster({ path, alt, person = false, priority = false, className, badges }: { path?: string; alt: string; person?: boolean; priority?: boolean; className?: string; badges?: React.ReactNode }) {
  ...
  return <div className={cn("relative aspect-2/3 overflow-hidden bg-muted", className)}>
    {path && failedPath !== path
      ? <><div className={`absolute inset-0 animate-pulse bg-muted ${loadedPath === path ? "hidden" : ""}`} /><Image src={`https://image.tmdb.org/t/p/w500${path}`} alt={alt} fill sizes="(max-width: 640px) 45vw, (max-width: 1280px) 25vw, 220px" priority={priority} onLoad={() => setLoadedPath(path)} onError={() => setFailedPath(path)} className={`object-cover transition-opacity duration-300 ${loadedPath === path ? "opacity-100" : "opacity-0"}`} /></>
    ...
    {badges && <div className="absolute inset-x-0 top-0 flex flex-wrap items-start justify-between gap-1.5 bg-linear-to-b from-black/65 to-transparent p-2 text-white">{badges}</div>}
  </div>;
}
