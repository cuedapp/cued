...
export default async function UserProfilePage({
  ...
  const requestTitles = requests.items.map(({ request }) => ({
    type: request.mediaType === "series" ? ("series" as const) : ("movie" as const),
    tmdbId: request.tmdbId,
  }));
  const availableKeys = new Set(await libraryService.getAvailableKeys(id, requestTitles));
  const items = await Promise.all(
    requests.items.map(async ({ request, reviewerName }) => {
      const mediaType = request.mediaType === "series" ? ("series" as const) : ("movie" as const);
      const title = await tmdbMetadataService
        .getTitle(currentUser.id, mediaType, request.tmdbId, locale)
        .catch(() => undefined);
      const displayStatus =
        request.status === "approved"
          ? availableKeys.has(`${mediaType}:${request.tmdbId}`)
            ? ("completed" as const)
            : ("requested" as const)
          : request.status;
      return {
        ...
        mediaType,
        displayStatus,
        title: title?.title ?? t("unknownTitle", { id: request.tmdbId }),
      };
    }),
  );
  ...
  return (
    <div className="space-y-8">
      <header className="flex flex-col gap-5 rounded-3xl border border-border bg-card p-6 sm:flex-row sm:items-center">
        <UserAvatar
          userId={profile.id}
          name={profile.displayName}
          avatarTag={profile.primaryImageTag}
          className="size-20"
        />
      ...
      <section className="space-y-4">
        ...
        <div className="grid gap-5 sm:grid-cols-2 xl:grid-cols-4">
          <ProfileStat
            ...
            label={t("watchTime")}
            value={activityT(watchTime.unit, { hours: watchTime.value, days: watchTime.value, weeks: watchTime.value })}
          />
          <ProfileStat
            icon={<BarChart3 className="size-4" />}
            label={t("watchedTitles")}
            value={statistics?.watchedTitles ?? 0}
          />
          <ProfileStat
            icon={<Star className="size-4" />}
            label={t("ratings")}
            value={
              statistics?.averageRating === null || !statistics
                ? t("noRatings")
                : t("rating", { rating: statistics.averageRating, count: statistics.ratings })
            }
          />
            ...
            {items.map(({ request, reviewerName, mediaType, displayStatus, title }) => {
              ...
              const StatusIcon =
                displayStatus === "pending"
                  ? Clock3
                  : displayStatus === "completed" || displayStatus === "requested"
                    ? CheckCircle2
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex flex-wrap items-start justify-between gap-3">
                        <div>
                          <Link
                            href={`/title/${mediaType}/${request.tmdbId}` as never}
                      ...
                      {request.reviewedAt && (
                        <p className="mt-3 text-sm text-muted-foreground">
                          {t("reviewed", {
                            user: reviewerName ?? t("unknownReviewer"),
                            date: formatRelativeDateTime(
                              request.reviewedAt,
                              new Date(),
                              locale,
...
function ProfileStat({ icon, label, value }: { icon: React.ReactNode; label: string; value: React.ReactNode }) {
  return (
    <Card>
