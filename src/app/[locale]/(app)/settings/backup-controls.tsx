...
export function BackupControls({ isAdmin }: { isAdmin: boolean }) {
  ...
  async function uploadArchive(file: File, kind: "user" | "full") {
    ...
    try {
      ...
      if (kind === "full") { toast.success(t("restored")); window.location.assign(window.location.href); return; }
...
function postArchive(kind: "user" | "full", file: File, onProgress: (progress: number) => void): Promise<{ result?: { feedback: number; follows: number; skipped: number } | string; error?: string }> {
  return new Promise((resolve, reject) => {
    ...
    const formData = new FormData(); formData.set("archive", file); request.send(formData);
  });
}
