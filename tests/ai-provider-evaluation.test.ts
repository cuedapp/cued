...
describe.each([
    ...
    (outputs: unknown[]) =>
      new OpenAiClient(
        vi.fn<typeof fetch>().mockImplementation(async () =>
          json({
            output: [{ type: "message", content: [{ type: "output_text", text: JSON.stringify(outputs.shift()) }] }],
          }),
        ),
      ),
  ],
      ...
      new OpenRouterClient(
        vi
          ...
          .mockImplementation(async () =>
            json({ choices: [{ message: { content: JSON.stringify(outputs.shift()) } }] }),
          ),
      ),
  ],
  ...
  it("produces the same validated profile and reranking contract", async () => {
    ...
    const reranked = await provider.rerank(
      ...
      fixture.candidates as AiCandidate[],
    );
    expect(profile).toEqual(fixture.profile);
    expect(reranked).toEqual(fixture.reranked);
    expect(new Set(reranked.map((item) => `${item.type}:${item.id}`))).toEqual(
      new Set(fixture.candidates.map((item) => `${item.type}:${item.id}`)),
    );
  });
});
