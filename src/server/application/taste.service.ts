...
export class TasteService {
  ...
  async getHistory(userId: string) {
    ...
    const resolved = await Promise.all(
      history.map(async (item) => {
        ...
        const lastEpisodePlayedAt = episodes.reduce<Date | null>(
          (latest, episode) =>
            episode.lastPlayedAt && (!latest || episode.lastPlayedAt > latest) ? episode.lastPlayedAt : latest,
          null,
        );
        return {
          ...item,
    ...
    return resolved
      .filter((item) => item.played || (item.playedPercentage ?? 0) > 0)
  ...
  async saveFeedback(
    ...
    if (!(await this.repository.isInUserHistory(userId, mediaItemId)))
      throw new Error("Media is not in this user's history");
