...
export const acquisitionRequestStatus = pgEnum("acquisition_request_status", [
  "pending",
  "approved",
  "rejected",
  "failed",
...
export const externalMediaAvailability = pgTable(
  "external_media_availability",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    integrationId: uuid("integration_id")
      .notNull()
  ...
  (table) => [
    uniqueIndex("external_media_availability_provider_source_idx").on(
      table.integrationId,
      table.mediaType,
      table.externalId,
    ),
...
export const mediaRatingRefreshStates = pgTable(
  "media_rating_refresh_states",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    mediaType: mediaKind("media_type").notNull(),
    tmdbId: integer("tmdb_id").notNull(),
...
export const userMediaFeedback = pgTable(
  "user_media_feedback",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    userId: uuid("user_id")
...
export const recommendationRuns = pgTable(
  ...
  (table) => [
    ...
    uniqueIndex("recommendation_runs_user_running_idx")
      .on(table.userId)
      .where(sql`${table.status} = 'running'`),
  ],
);

export const userMediaStates = pgTable(
  "user_media_states",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    userId: uuid("user_id")
...
export const acquisitionRequests = pgTable(
  ...
  (table) => [
    ...
    uniqueIndex("acquisition_requests_pending_title_idx")
