...
describe("TasteService", () => {
  it("only saves feedback for media in the current user's history", async () => {
    const repository = {
      isInUserHistory: vi.fn().mockResolvedValue(true),
      saveFeedback: vi.fn().mockResolvedValue({ id: "feedback" }),
    } as unknown as TasteRepository;
    const service = new TasteService(repository);
    await expect(service.saveFeedback("user", "media", { rating: 5, tags: [], excluded: false })).resolves.toEqual({
      id: "feedback",
    });
  ...
  it("rejects feedback for media outside the current user's history", async () => {
    ...
    await expect(
      new TasteService(repository).saveFeedback("user", "media", { tags: [], excluded: false }),
  ...
  it("derives series completion and recency from released episode activity", async () => {
    ...
    const repository = {
      getHistory: vi.fn().mockResolvedValue([
        {
          id: "series",
          jellyfinItemId: "jf-series",
          kind: "series",
          played: false,
          playedPercentage: 0,
          lastPlayedAt: oldSeriesDate,
        },
      ]),
  ...
  it("keeps watched seasons available for optional ratings", async () => {
    const season = {
      id: "season",
      jellyfinItemId: "jf-season",
      kind: "season",
      played: true,
      playedPercentage: 100,
      lastPlayedAt: new Date("2026-08-20T18:00:00Z"),
    };
