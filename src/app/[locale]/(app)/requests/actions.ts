...
export interface ReviewRequestState { result?: "approved" | "rejected"; error?: "failed" }
export async function reviewAcquisitionRequest(_: ReviewRequestState, formData: FormData): Promise<ReviewRequestState> {
  ...
  const input = z.object({ id: z.string().uuid(), locale: z.string().refine(isLocale), decision: z.enum(["approved", "rejected"]), rootFolderPath: z.string().optional(), qualityProfileId: z.coerce.number().int().positive().optional() }).safeParse({ id: formData.get("id"), locale: formData.get("locale"), decision: formData.get("decision"), rootFolderPath: formData.get("rootFolderPath") || undefined, qualityProfileId: formData.get("qualityProfileId") || undefined });
  ...
  try {
    if (input.data.decision === "approved") {
      ...
      await acquisitionService.approve(input.data.id, user.id, { rootFolderPath: input.data.rootFolderPath, qualityProfileId: input.data.qualityProfileId });
    }
