...
export default async function TitlePage({ params }: { params: Promise<{ type: string; id: string }> }) {
  ...
  return (
    <div className="space-y-10">
      <section className="relative -mx-5 -mt-5 overflow-hidden border-b border-border/60 sm:-mx-8 sm:-mt-8 lg:-mx-12 lg:-mt-12">
        {title.backdropPath && (
          <div className="absolute inset-0">
            <Image
              src={tmdbImageUrl(title.backdropPath, "original")}
              alt=""
              fill
              priority
              sizes="100vw"
        ...
        <div className="absolute left-5 top-5 z-10 rounded-lg bg-background/70 px-3 py-2 backdrop-blur-sm sm:left-8 sm:top-8 lg:left-12 lg:top-12">
          <BackButton />
        </div>
        <div className="relative flex w-full flex-col gap-8 px-5 pb-12 pt-16 sm:px-8 md:flex-row md:items-end lg:px-12 lg:pt-28">
          <MediaPoster
            ...
            alt={title.title}
            priority
            className="w-40 shrink-0 rounded-2xl shadow-2xl sm:w-52"
          />
          <div className="max-w-3xl pb-2">
            <div className="flex flex-wrap items-center gap-3 text-sm text-muted-foreground">
              ...
              {title.runtimeMinutes && (
                <span className="inline-flex items-center gap-1">
                  ...
                  {t("minutes", { count: title.runtimeMinutes })}
                </span>
              )}
            </div>
            ...
            <div className="mt-5">
              <MediaRatings
                ...
                labels={{
                  ...
                  metacritic: libraryT("ratingSources.metacritic"),
                  tmdb: libraryT("ratingSources.tmdb"),
                  trakt: libraryT("ratingSources.trakt"),
                }}
              />
            </div>
            <div className="mt-5 flex items-center gap-2">
              <MediaCapabilityBadges
                ...
                strmPending={title.strmPending}
                strmRequestable={title.m3uAvailable}
            ...
            {title.strmPending && (
              <p
                className="mt-3 max-w-xl rounded-lg border border-amber-500/30 bg-amber-500/10 px-3 py-2 text-sm leading-6 text-foreground"
                role="status"
              >
                {t("strmPendingDetail")}
              </p>
            ...
            <div className="mt-5 flex flex-wrap gap-2">
              ...
              {(acquisition.configured || title.m3uAvailable) && (
                <RequestButton
                  ...
                  strmAvailable={title.m3uAvailable && !title.available && !title.strmAvailable && !title.strmPending}
                  strmAlreadyAvailable={title.strmAvailable}
                  strmImportPending={title.strmPending}
                  options={{
                  ...
                  initialState={
                    title.available
                      ? "available"
                      : acquisitionState === "existing" || acquisitionState === "pending"
                        ? acquisitionState
                        : "idle"
                  }
                />
            ...
            <div className="mt-5 flex flex-wrap gap-2">
              {title.genres.map((genre) => (
                <span key={genre.id} className="rounded-full border border-border bg-background/60 px-3 py-1 text-xs">
                  {genre.name}
                </span>
              ))}
            </div>
      ...
      {recommendation && (
        <section className="max-w-4xl rounded-2xl border border-primary/25 bg-primary/5 p-6">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div className="flex items-start gap-3">
              ...
              <div>
                <p className="text-xs font-bold uppercase tracking-[0.16em] text-primary">{t("recommendedForYou")}</p>
                <h2 className="mt-1 font-display text-2xl font-semibold">{t("recommendationDetails")}</h2>
              </div>
            </div>
          ...
          <div className="mt-5 space-y-2 text-sm leading-6 text-muted-foreground">
            {recommendation.aiExplanation && (
              <div className="rounded-xl border border-primary/20 bg-background/50 p-4">
                <p className="text-sm font-medium text-foreground">{recommendationCardT("aiReason")}</p>
                <p className="mt-1 text-foreground">{recommendation.aiExplanation}</p>
              </div>
            )}
            {likedSources.length > 0 && (
              <p>
                {recommendationT("becauseTitles", { titles: likedSources.map((source) => source.title).join(", ") })}
              </p>
            )}
            {watchedSources.length > 0 && (
              <p>
                {recommendationT("becauseWatched", { titles: watchedSources.map((source) => source.title).join(", ") })}
              </p>
            )}
            {likedSources.length === 0 && watchedSources.length === 0 && recommendation.reasons.length > 0 && (
              <p>{recommendationT("becauseGenres", { genres: recommendation.reasons.join(", ") })}</p>
            )}
          </div>
        </section>
      )}
      ...
      {historyItem && (
        <section className="w-full rounded-2xl border border-border/70 bg-card p-6">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <h2 className="font-display text-2xl font-semibold tracking-tight">{t("yourRating")}</h2>
            {historyItem.lastPlayedAt && (
              ...
              >
                ...
                {historyT("watchedAt", {
                  date: formatRelativeDate(historyItem.lastPlayedAt, new Date(), locale, user.dateFormat),
                  time: formatDisplayTime(historyItem.lastPlayedAt, user.timeFormat, locale),
                })}
              </div>
            )}
          </div>
          <RatingForm
            mediaItemId={historyItem.id}
            rating={historyItem.rating}
            feedback={historyItem.feedback}
            tags={historyItem.tags ?? []}
            excluded={historyItem.excluded}
          />
        </section>
      )}
          ...
          <div className="mt-4 flex flex-wrap gap-3">
            {title.crew.map((person) => (
              ...
              >
                ...
                <span className="ml-2 text-sm text-muted-foreground">{person.role}</span>
              </Link>
            ))}
          </div>
        </section>
