...
export function M3uEditorForm({
  ...
  const [storedPlaylists] = useState<Playlist[]>(
    overview.playlists.length
      ? overview.playlists
      : overview.playlistUuid
        ? [{ uuid: overview.playlistUuid, name: t("selectedPlaylist") }]
        : [],
  );
        ...
        {libraries.map((library) => (
          ...
          >
            <input
              ...
              onChange={(event) =>
                onChange(event.target.checked ? [...selected, library.id] : selected.filter((id) => id !== library.id))
              }
