...
import { acquisitionService, activityService, radarrIntegrationService, recommendationService, sonarrIntegrationService } from "@/server/application/services";
...
export default async function Dashboard() {
  ...
  const [radarr, sonarr] = user ? await Promise.all([radarrIntegrationService.getOverview(), sonarrIntegrationService.getOverview()]) : [undefined, undefined];
  const allowRequestOptions = Boolean(user && (user.role === "admin" || !user.requestsRequireApproval));
  const [radarrOptions, sonarrOptions] = user && allowRequestOptions ? await Promise.all([
  ...
  const requestStates = user ? await acquisitionService.getStates(recommendations.map((item) => ({ type: item.mediaType as "movie" | "series", tmdbId: item.tmdbId }))).catch(() => ({} as Record<string, "idle" | "pending" | "existing">)) : {};
  const requestContext = { requestable: { movie: radarr?.configured ?? false, series: sonarr?.configured ?? false }, options: { movie: { rootFolders: radarrOptions.rootFolders, profiles: radarrOptions.qualityProfiles, defaultRootFolderPath: radarr?.rootFolderPath, defaultProfileId: radarr?.qualityProfileId }, series: { rootFolders: sonarrOptions.rootFolders, profiles: sonarrOptions.qualityProfiles, defaultRootFolderPath: sonarr?.rootFolderPath, defaultProfileId: sonarr?.qualityProfileId } }, allowOptions: allowRequestOptions, states: requestStates };
    ...
    <div className="space-y-8">
      <section className="relative overflow-hidden rounded-4xl border border-border/60 bg-card px-6 py-10 shadow-sm sm:px-10 sm:py-14">
        ...
        <div className="relative max-w-2xl">
          ...
          <h1 className="font-display text-5xl font-semibold tracking-tighter sm:text-6xl"><DashboardGreeting /></h1>
      ...
      <section className="space-y-5"><div className="flex flex-wrap items-end justify-between gap-4"><div><h2 className="font-display text-3xl font-semibold tracking-tight">{t("recommendationsTitle")}</h2><p className="mt-1 text-sm text-muted-foreground">{t("recommendationsBody")}</p></div><div className="flex items-center gap-3"><Link href="/recommendations" className="text-sm font-medium text-primary hover:underline">{t("viewAll")}</Link><RecommendationRefreshButton /></div></div>
        {recommendations.length === 0 ? <Card className="p-8 text-center"><CardTitle>{t("emptyTitle")}</CardTitle><CardDescription className="mt-2">{t("emptyBody")}</CardDescription></Card> : <div className="space-y-8">{movieRecommendations.length > 0 && <RecommendationSection title={t("moviesForYou")} items={movieRecommendations} requestContext={requestContext} />}{seriesRecommendations.length > 0 && <RecommendationSection title={t("seriesForYou")} items={seriesRecommendations} requestContext={requestContext} />}</div>}
      </section>

      {hiddenRecommendations.length > 0 && <details className="rounded-2xl border border-border bg-card"><summary className="cursor-pointer px-5 py-4 text-sm font-medium">{t("hiddenRecommendations", { count: hiddenRecommendations.length })}</summary><div className="grid gap-3 border-t border-border p-4 sm:grid-cols-2">{hiddenRecommendations.map((item) => <div key={item.id} className="flex items-center justify-between gap-3 rounded-xl border border-border p-3"><span className="truncate text-sm font-medium">{item.title}</span><form action={async (formData) => { await updateRecommendationFeedback(formData); }}><input type="hidden" name="recommendationId" value={item.id} /><button name="feedback" value="restore" className="inline-flex cursor-pointer items-center gap-1 text-sm text-primary hover:underline"><Undo2 className="size-4" />{t("restore")}</button></form></div>)}</div></details>}
...
function ServerActivity({ activity, locale, dateFormat, t: translate }: { activity: DashboardActivity; locale: string; dateFormat: string; t: Awaited<ReturnType<typeof getTranslations<"Activity">>> }) {
  const t = (key: string, values?: Record<string, string | number>) => key === "episode"
    ? `${values?.season ?? "–"}x${values?.episode ?? "–"} · ${values?.title ?? ""}`
    : (translate as (translationKey: string, translationValues?: Record<string, string | number>) => string)(key, values);
...
type DashboardRequestContext = { requestable: { movie: boolean; series: boolean }; options: { movie: RequestOptions; series: RequestOptions }; allowOptions: boolean; states: Record<string, "idle" | "pending" | "existing"> };

async function RecommendationSection({ title, items, requestContext }: { title: string; items: RecommendationItem[]; requestContext: DashboardRequestContext }) {
  const t = await getTranslations("Dashboard");
  return <section><h3 className="mb-4 text-sm font-bold uppercase tracking-[0.16em] text-muted-foreground">{title}</h3><div className="grid grid-cols-2 gap-4 sm:grid-cols-3 xl:grid-cols-6">{items.map((item) => { const liked = item.sourceTitles.filter((source) => source.reason === "liked"); const watched = item.sourceTitles.filter((source) => source.reason === "watched"); const requestable = requestContext.requestable[item.mediaType as "movie" | "series"]; const hasRequest = requestable || item.m3uAvailable; return <RecommendationGridCard key={item.id} item={item} labels={{ available: t("available"), strmAvailable: t("strmAvailable"), strmPending: t("strmPending"), strmRequestable: t("strmRequestable"), type: t(`types.${item.mediaType}`), becauseLiked: liked.length > 0 ? t("becauseTitles", { titles: liked.map((source) => source.title).join(", ") }) : undefined, becauseWatched: watched.length > 0 ? t("becauseWatched", { titles: watched.map((source) => source.title).join(", ") }) : undefined, becauseGenres: item.sourceTitles.length === 0 && item.reasons.length > 0 ? t("because", { reasons: item.reasons.join(", ") }) : undefined }} request={hasRequest ? { type: item.mediaType as "movie" | "series", tmdbId: item.tmdbId, options: requestContext.options[item.mediaType as "movie" | "series"], allowOptions: requestContext.allowOptions, arrAvailable: requestable, strmAvailable: item.m3uAvailable && !item.available && !item.strmAvailable && !item.strmPending, strmAlreadyAvailable: item.strmAvailable, strmImportPending: item.strmPending, initialState: item.available ? "available" : requestContext.states[`${item.mediaType}:${item.tmdbId}`] ?? "idle" } : undefined} />; })}</div></section>;
}
