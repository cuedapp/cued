...
export const integrationsRouter = createRouter({
  ...
  selectJellyfinLibraries: adminProcedure
    .input(z.object({ libraryIds: z.array(z.string()) }))
    .mutation(({ ctx, input }) => ctx.services.jellyfinIntegration.selectLibraries(input.libraryIds)),
  syncJellyfin: adminProcedure
    .input(z.object({ mode: z.enum(["full", "updates"]) }).optional())
    .mutation(({ ctx, input }) => {
      if (!ctx.services.mediaSync)
        throw new TRPCError({ code: "PRECONDITION_FAILED", message: "Secret encryption is not configured" });
  ...
  configureTmdb: adminProcedure
    .input(z.object({ accessToken: z.string().optional() }))
    .mutation(({ ctx, input }) => ctx.services.tmdbIntegration.configure(input.accessToken)),
