...
describe("ArrClient", () => {
  it("discovers server configuration without exposing the key in the URL", async () => {
    const transport = vi.fn().mockResolvedValueOnce(json({ instanceName: "Movies", version: "6.0.0" })).mockResolvedValueOnce(json([{ id: 1, path: "/movies" }])).mockResolvedValueOnce(json([{ id: 2, name: "HD" }]));
    const client = new ArrClient("radarr", transport);
    await expect(Promise.all([client.getStatus(connection), client.getRootFolders(connection), client.getQualityProfiles(connection)])).resolves.toEqual([{ instanceName: "Movies", version: "6.0.0" }, [{ id: 1, path: "/movies" }], [{ id: 2, name: "HD" }]]);
  ...
  it("looks up and requests a movie with the configured defaults", async () => {
    ...
    const transport = vi.fn().mockResolvedValueOnce(json(movie)).mockResolvedValueOnce(json({ ...movie, id: 4 }));
    const client = new ArrClient("radarr", transport);
    ...
    expect(JSON.parse(transport.mock.calls[1]?.[1]?.body as string)).toMatchObject({ rootFolderPath: "/movies", qualityProfileId: 2, monitored: true, addOptions: { searchForMovie: true } });
  });

  it("loads the provider inventory for request-state checks", async () => {
    const transport = vi.fn().mockResolvedValue(json([{ id: 4, tmdbId: 10, title: "Movie" }, { id: 5, tmdbId: 20, title: "Other" }]));
  ...
  it("maps provider errors to a safe request error", async () => {
    ...
    await expect(client.getStatus(connection)).rejects.toEqual(expect.objectContaining<Partial<ArrRequestError>>({ status: 401, message: "Invalid API key" }));
  });
});
