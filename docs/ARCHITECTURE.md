...
The Library page queries only selected Jellyfin libraries accessible to the signed-in user. Movies and series are paginated in the database and can be filtered by type and active/removed state. Removed rows retain their stable media identity and are shown without attempting a Jellyfin image request; a later sync upserts the same provider identity and clears `removed_at`. The Library and Recommendations pages reuse one browser-local viewing-intent control, which reorders the displayed results without persisting or changing taste signals.
