      ...
      {items.length === 0 ? (
        ...
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6">
          {items.map((item) => {
                  ...
                  <div className="mt-1 flex items-center gap-2 text-xs text-muted-foreground">
                    ...
                    {item.rating && (
                      ...
                      >
                        ...
                        {t(`ratingSources.${item.rating.source}`)} {formatRating(item.rating.value, item.rating.scale)}
                      </span>
                    )}
                  </div>
                  {item.overview && (
            ...
            return (
              <article key={item.id} className="group overflow-hidden rounded-2xl border border-border bg-card">
                {item.tmdbId ? (
                  <Link
                    href={`/title/${item.mediaType}/${item.tmdbId}` as never}
                    className="block h-full outline-none ring-offset-2 focus-visible:ring-2 focus-visible:ring-ring"
                  >
                    {body}
                  </Link>
                ) : (
