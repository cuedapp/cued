...
describe("SeriesProgressService", () => {
  it("loads a user's persisted episodes before calculating completion", async () => {
    const repository = { getSeriesEpisodes: vi.fn().mockResolvedValue([{ played: true }, { played: false }]) } as unknown as MediaSyncRepository;
    await expect(new SeriesProgressService(repository).getCompletion("user", "series", new Date())).resolves.toEqual({ played: 1, released: 2, percentage: 50 });
