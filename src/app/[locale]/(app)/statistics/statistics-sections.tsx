...
type StatisticsUser = NonNullable<Awaited<ReturnType<typeof getCurrentUser>>>;

export function StatisticsSectionLoading({ cards = 1 }: { cards?: number }) {
  return (
...
export async function ServerStatisticsSection() {
  const [t, activityT, server] = await Promise.all([
...
export async function UserStatisticsSection({ user, selectedUser }: { user: StatisticsUser; selectedUser?: string }) {
  const [t, activityT, locale, summaries] = await Promise.all([
    getTranslations("Statistics"),
    getTranslations("Activity"),
    getLocale(),
    activityService.getAdminUserSummaries(),
  ]);
  const selectedUserId = summaries.some((summary) => summary.id === selectedUser) ? selectedUser : undefined;
  const visibleSummaries = selectedUserId ? summaries.filter((summary) => summary.id === selectedUserId) : [];
  const [selectedTrend, insights] = await Promise.all([
    selectedUserId ? activityService.getStatisticsTrend(selectedUserId) : Promise.resolve(null),
    activityService.getStatisticsInsights(selectedUserId),
  ]);
  return (
    <section className="space-y-4">
      ...
      <div className="flex gap-1 overflow-x-auto rounded-xl bg-muted p-1" role="tablist" aria-label={t("userFilter")}>
