...
export class AiRepository {
  async getIntegration(provider: AiProviderId = "openai") { return db.query.integrations.findFirst({ where: eq(integrations.provider, provider) }); }

  async getEnabledIntegration() {
    const rows = await db.query.integrations.findMany({ where: sql`${integrations.provider} in ('openai', 'openrouter')`, orderBy: (integration, { desc }) => [desc(integration.updatedAt)] });
    return rows.find((row) => ((row.configuration as { mode?: AiMode }).mode ?? "off") !== "off");
  }

  async saveIntegration(provider: AiProviderId, encryptedApiKey: string | null, mode: AiMode, model: string, refreshDelayMinutes = 5) {
    const now = new Date();
    ...
    const details = provider === "openrouter" ? { baseUrl: "https://openrouter.ai/api/v1", serverName: "OpenRouter" } : { baseUrl: "https://api.openai.com/v1", serverName: "OpenAI" };
    const [saved] = await db.insert(integrations).values({ provider, ...details, encryptedApiKey, configuration, status: mode === "off" ? "unconfigured" : "healthy", lastCheckedAt: mode === "off" ? null : now, updatedAt: now }).onConflictDoUpdate({ target: integrations.provider, set: { ...details, encryptedApiKey, configuration, status: mode === "off" ? "unconfigured" : "healthy", lastError: null, updatedAt: now } }).returning();
    if (!saved) throw new Error("AI integration could not be saved");
    if (mode !== "off") await db.update(integrations).set({ configuration: sql`${integrations.configuration} || '{"mode":"off"}'::jsonb`, status: "unconfigured", updatedAt: now }).where(and(sql`${integrations.provider} in ('openai', 'openrouter')`, ne(integrations.provider, provider)));
    return saved;
  }

  async recordUsage(provider: AiProviderId, usage: { model: string; inputTokens: number; outputTokens: number; costUsd?: number }) {
  ...
  async getCached<T>(provider: AiProviderId, key: string): Promise<T | undefined> {
    const row = await db.query.metadataCacheEntries.findFirst({ where: and(eq(metadataCacheEntries.provider, provider), eq(metadataCacheEntries.cacheKey, key), gt(metadataCacheEntries.expiresAt, new Date())) });
