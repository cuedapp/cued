...
describe("JellyfinClient", () => {
  ...
  it("authenticates a Jellyfin administrator without retaining the password", async () => {
    const transport = vi.fn<typeof fetch>().mockResolvedValue(jsonResponse({
      ...
      User: { Id: "user-1", Name: "Erik", PrimaryImageTag: "avatar-tag", Policy: { IsAdministrator: true, IsDisabled: false } },
    }));
    const result = await new JellyfinClient("http://jellyfin:8096", transport).authenticate("Erik", "password");
    expect(result).toEqual({ serverId: "server-1", accessToken: "access-token", user: { id: "user-1", username: "Erik", primaryImageTag: "avatar-tag", isAdministrator: true, isDisabled: false, hasAccessToAllLibraries: true, enabledLibraryIds: [] } });
  ...
  it("proxies a user avatar without placing the API key in the URL", async () => {
    const transport = vi.fn<typeof fetch>().mockResolvedValue(new Response(new Uint8Array([1, 2, 3]), { headers: { "Content-Type": "image/jpeg", ETag: "avatar-tag" } }));
    const avatar = await new JellyfinClient("http://jellyfin:8096", transport).getUserAvatar("api-key", "user-1", "avatar-tag");
  ...
  it("returns a sanitized error when Jellyfin rejects a request", async () => {
    ...
    await expect(new JellyfinClient("http://jellyfin:8096", transport).getUsers("bad-key")).rejects.toEqual(expect.objectContaining<JellyfinRequestError>({ name: "JellyfinRequestError", status: 401, message: "Jellyfin request failed" }));
  });
