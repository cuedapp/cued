...
export function TmdbIntegrationForm({ locale, encryptionConfigured, hasAccessToken }: { locale: string; encryptionConfigured: boolean; hasAccessToken: boolean }) {
  ...
  return <form action={action} className="space-y-5">
    ...
    <div className="space-y-2">
      ...
      <Input id="tmdbAccessToken" name="accessToken" type="password" placeholder={hasAccessToken ? t("tokenStored") : undefined} disabled={!encryptionConfigured} autoComplete="off" />
    ...
    <div className="flex flex-wrap gap-3">
      ...
      <FormSubmitButton name="intent" value="test" variant="outline" pendingLabel={t("testing")} disabled={!hasAccessToken}>{t("test")}</FormSubmitButton>
    </div>
  </form>;
}
