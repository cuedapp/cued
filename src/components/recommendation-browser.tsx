    ...
    <section className="overflow-hidden rounded-2xl border border-border bg-card">
      ...
      <div className="flex flex-col gap-3 border-t border-border/70 bg-muted/20 px-4 py-3 sm:flex-row sm:items-center sm:justify-between sm:px-5">
        ...
        <div className="flex flex-col gap-2 sm:flex-row">
          ...
          <button type="button" onClick={() => setFreshDialogOpen(true)} disabled={busy} className="inline-flex h-9 cursor-pointer items-center justify-center gap-2 rounded-lg px-3.5 text-sm font-medium text-destructive hover:bg-destructive/10 disabled:cursor-wait disabled:opacity-60">{startingFresh ? <RefreshCw className="size-4 animate-spin" /> : <Trash2 className="size-4" />}{t("startFresh")}</button>
        </div>
      </div>
    </section>
...
function RecommendationGridItem({ item, requestable, options, allowOptions, initialState }: { item: Item; requestable: boolean; options?: RequestOptions; allowOptions: boolean; initialState: "idle" | "pending" | "existing" }) {
  const t = useTranslations("Recommendations");
  ...
  return <RecommendationGridCard item={item} labels={{ available: t("available"), strmAvailable: t("strmAvailable"), strmPending: t("strmPending"), strmRequestable: t("strmRequestable"), type: t(item.mediaType === "movie" ? "movie" : "series"), becauseLiked: liked.length > 0 ? t("becauseTitles", { titles: liked.map((source) => source.title).join(", ") }) : undefined, becauseWatched: watched.length > 0 ? t("becauseWatched", { titles: watched.map((source) => source.title).join(", ") }) : undefined, becauseGenres: item.sourceTitles.length === 0 && item.reasons.length > 0 ? t("becauseGenres", { genres: item.reasons.join(", ") }) : undefined }} request={hasRequest ? { type: item.mediaType as "movie" | "series", tmdbId: item.tmdbId, options, allowOptions, arrAvailable: requestable, strmAvailable: item.m3uAvailable && !item.available && !item.strmAvailable && !item.strmPending, strmAlreadyAvailable: item.strmAvailable, strmImportPending: item.strmPending, initialState: item.available ? "available" : initialState } : undefined} />;
}
