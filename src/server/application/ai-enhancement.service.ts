...
export class AiEnhancementService {
  ...
  getRefreshDelayMinutes() {
    return this.integration.getRefreshDelayMinutes();
  }
  ...
  async enhance(userId: string, locale: string, signals: RecommendationSignal[], candidates: ScoredRecommendation[]) {
    ...
    try {
      ...
      const cacheKey = rerankFingerprint(
        createHash("sha256").update(JSON.stringify(profile)).digest("hex"),
        connection.model,
        locale,
        shortlist,
      );
      let reranked = await this.repository.getCached<{ recommendations: AiRerankedCandidate[] }>(
        connection.providerId,
        cacheKey,
      );
      if (!reranked) {
        logger.info("AI recommendation reranking started", {
          userId,
          model: connection.model,
          mode: connection.mode,
          candidateCount: shortlist.length,
        });
        const recommendations = await this.integration.execute((provider, apiKey, model) =>
          provider.rerank(
            apiKey,
            model,
            locale,
        ...
        logger.info("AI recommendation reranking cache hit", {
          userId,
          model: connection.model,
          candidateCount: shortlist.length,
        });
      ...
      const byTitle = new Map(
        reranked.recommendations.map((item) => [
          `${item.type}:${item.id}`,
          { ...item, score: normalizeAiScore(item.score) },
        ]),
      );
      ...
      const enhanced = candidates
        .map((candidate) => {
          ...
          return {
            ...
            matchPercent: Math.round(candidate.matchPercent * (1 - influence) + ai.score * influence),
            score: candidate.score + ai.score * influence,
          };
        })
        .sort(compareRecommendations);
      logger.info("AI recommendation enhancement completed", {
        userId,
        model: connection.model,
        mode: connection.mode,
        candidateCount: shortlist.length,
      ...
      logger.warn("AI recommendation enhancement failed; deterministic fallback used", {
        userId,
        model: connection.model,
        mode: connection.mode,
  ...
  async refreshProfile(userId: string, locale: string, signals: RecommendationSignal[]) {
    return this.getProfile(userId, locale, signals, true);
  }
  ...
  private async getProfile(
    ...
    force: boolean,
  ): Promise<{ profile: TasteProfile; fingerprint: string }> {
    const connection = await this.integration.getConnection();
    if (!connection) throw new Error("AI is disabled");
    const aiSignals: AiTasteSignal[] = signals.map((signal) => ({
      title: signal.title ?? "Untitled",
      type: signal.type,
      rating: signal.rating,
      tags: signal.tags ?? [],
      ...(signal.feedback ? { feedback: signal.feedback } : {}),
      watched: signal.played,
    }));
    ...
    if (
      !force &&
      existing?.signalFingerprint === fingerprint &&
      existing.provider === connection.providerId &&
      existing.model === connection.model
    ) {
    ...
    const startedAt = Date.now();
    logger.info("AI taste profile generation started", {
      userId,
      model: connection.model,
      signalCount: aiSignals.length,
      forced: force,
    });
    const profile = await this.integration.execute((provider, apiKey, model) =>
      provider.generateTasteProfile(apiKey, model, locale, aiSignals),
    );
    ...
    logger.info("AI taste profile generation completed", {
      userId,
      model: connection.model,
      signalCount: aiSignals.length,
      traitCount: profile.traits.length,
      dislikeCount: profile.dislikes.length,
      durationMs: Date.now() - startedAt,
    });
...
function rerankFingerprint(
  profileFingerprint: string,
  model: string,
  locale: string,
  candidates: ScoredRecommendation[],
) {
  return `rerank:${createHash("sha256")
    .update(
      JSON.stringify({
        version: 2,
        profileFingerprint,
        model,
        locale,
        candidates: candidates.map((item) => [item.type, item.id, item.matchPercent]),
      }),
    )
    .digest("hex")}`;
}
...
function selectBalancedShortlist(candidates: ScoredRecommendation[], limit: number) {
  ...
  selected.push(
    ...candidates
      .filter((candidate) => !selectedKeys.has(`${candidate.type}:${candidate.id}`))
