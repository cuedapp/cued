...
import { AppStatusProvider } from "./app-status-provider";
...
export function AppShell({
  ...
  return (
    <AppStatusProvider>
      <div
        className={cn("min-h-dvh lg:grid", sidebarCollapsed ? "lg:grid-cols-[72px_1fr]" : "lg:grid-cols-[264px_1fr]")}
      >
        <a
          href="#main-content"
        ...
        <div className="min-w-0">
          <header className="sticky top-0 z-20 border-b border-border/60 bg-background/85 backdrop-blur-xl">
            <div className="flex h-16 items-center gap-3 px-5 lg:px-8 xl:px-12">
              ...
              <form onSubmit={submitMobileSearch} className="relative mr-auto hidden w-full max-w-md lg:block">
              ...
              <div className="ml-auto flex items-center gap-1 sm:gap-2">
                <button
                  type="button"
                  className="grid size-10 shrink-0 cursor-pointer place-items-center rounded-lg text-muted-foreground outline-none ring-offset-2 transition-colors hover:bg-accent hover:text-foreground focus-visible:ring-2 focus-visible:ring-ring lg:hidden"
                  aria-label={t("Search.label")}
    ...
    <AriaMenuTrigger>
      ...
      >
        <AriaMenu
          ...
          onAction={(key) => {
            if (key === "profile") router.push(`/profile/${user.id}` as never);
            if (key === "sign-out")
        ...
        >
          <AriaHeader className="px-3 py-2">
            <div className="truncate text-sm font-medium">{user.name}</div>
            <div className="text-xs font-normal text-muted-foreground">{t(`Roles.${user.role}`)}</div>
          </AriaHeader>
          <AriaSeparator className="my-1 h-px bg-border" />
          <AriaMenuItem
            id="profile"
            className="flex cursor-pointer items-center gap-2 rounded-lg px-3 py-2 text-sm outline-none hover:bg-accent focus:bg-accent"
          >
