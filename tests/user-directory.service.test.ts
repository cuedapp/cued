...
describe("UserDirectoryService", () => {
  it("maps each user's Jellyfin library permissions", async () => {
    const jellyfinRepository = { getIntegration: vi.fn().mockResolvedValue({ id: "integration" }) } as unknown as JellyfinRepository;
    const syncRepository = { getUsersWithLibraryAccess: vi.fn().mockResolvedValue({
