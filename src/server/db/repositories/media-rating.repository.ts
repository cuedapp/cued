...
export class MediaRatingRepository {
  async hasDue(staleBefore: Date) {
    const rows = await this.dueQuery(
      or(isNull(mediaRatingRefreshStates.id), lt(mediaRatingRefreshStates.lastAttemptAt, staleBefore)),
    ).limit(1);
  ...
  async getActiveRun() {
    return db.query.jobRuns.findFirst({
      where: and(eq(jobRuns.jobName, "media-rating-sync"), eq(jobRuns.status, "running")),
  ...
  async startRun(startedAt: Date) {
    const [run] = await db
      .insert(jobRuns)
      .values({ jobName: "media-rating-sync", status: "running", startedAt })
      .returning();
    return run!;
  }
  ...
  getRefreshState(mediaType: "movie" | "series", tmdbId: number) {
    return db.query.mediaRatingRefreshStates.findFirst({
      where: and(eq(mediaRatingRefreshStates.mediaType, mediaType), eq(mediaRatingRefreshStates.tmdbId, tmdbId)),
    });
  }
  ...
  async getRatings(mediaType: "movie" | "series", tmdbId: number): Promise<RatingValue[]> {
    const rows = await db
      .select({
        source: mediaRatings.source,
        value: mediaRatings.value,
        scale: mediaRatings.scale,
        normalizedScore: mediaRatings.normalizedScore,
        votes: mediaRatings.votes,
      })
      .from(mediaRatings)
  ...
  private dueQuery(refreshCondition: ReturnType<typeof or>) {
    return db
      .selectDistinct({
        ...
        lastAttemptAt: mediaRatingRefreshStates.lastAttemptAt,
      })
      .from(mediaItems)
      .leftJoin(
        mediaRatingRefreshStates,
        and(
  ...
  private mapTitles(
    rows: Array<{ mediaType: string; tmdbId: number | null }>,
  ): Array<{ mediaType: "movie" | "series"; tmdbId: number }> {
    return rows.flatMap((row) =>
      row.tmdbId !== null && (row.mediaType === "movie" || row.mediaType === "series")
  ...
  async save(mediaType: "movie" | "series", tmdbId: number, ratings: RatingValue[], attemptedAt: Date, error?: string) {
    ...
    await db
      .insert(mediaRatingRefreshStates)
      .values({
        mediaType,
        tmdbId,
...
function isRatingSource(value: string): value is RatingSource {
  return (
    value === "tmdb" || value === "imdb" || value === "rottenTomatoes" || value === "metacritic" || value === "trakt"
  );
}
