...
export class ReleaseService {
  ...
  async getStatus(force = false): Promise<ReleaseStatus> {
    ...
    if (!cached)
      try {
        const response = await this.request(releaseUrl, {
          headers: { Accept: "application/vnd.github+json", "User-Agent": "Cued" },
          signal: AbortSignal.timeout(10_000),
        });
        if (!response.ok) throw new Error(`GitHub returned ${response.status}`);
        const release = (await response.json()) as GithubRelease;
        if (release.prerelease) throw new Error("GitHub returned a prerelease");
        await this.repository.saveCachedRelease(
          { tagName: release.tag_name, url: release.html_url, notes: release.body, publishedAt: release.published_at },
          cacheTtlMs,
        );
        cached = await this.repository.getCachedRelease();
      } catch {
        /* An update check must never make Settings unavailable. */
      }
    const payload = cached?.payload as
      { tagName?: string; url?: string; notes?: string | null; publishedAt?: string | null } | undefined;
    const latestVersion = payload?.tagName ?? null;
    return {
      currentVersion: appVersion,
      latestVersion,
      updateAvailable: latestVersion ? isNewerVersion(latestVersion, appVersion) : false,
      releaseUrl: payload?.url ?? null,
