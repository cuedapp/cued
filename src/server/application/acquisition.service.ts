...
export class AcquisitionService {
  ...
  async request(
    ...
    if (user.role === "admin" || !user.requestsRequireApproval) {
      ...
      try {
        ...
        await this.repository.complete(request.id, user.id, "approved", {
  ...
  getPendingKeys(userId: string) {
    return this.repository.getPendingKeys(userId);
  }
  ...
  async getStates(items: Array<{ type: "movie" | "series"; tmdbId: number }>) {
    const [pendingKeys, movies, series] = await Promise.all([
      this.repository.getAllPendingKeys(),
      this.radarr.getExistingTmdbIds(),
      this.sonarr.getExistingTmdbIds(),
    ]);
    const pending = new Set(pendingKeys);
    const existingMovies = new Set(movies);
    const existingSeries = new Set(series);
    return Object.fromEntries(
      items.map(({ type, tmdbId }) => {
        const key = `${type}:${tmdbId}`;
        return [
          key,
          pending.has(key)
            ? "pending"
            : (type === "movie" ? existingMovies : existingSeries).has(tmdbId)
              ? "existing"
              : "idle",
        ];
      }),
  ...
  async approve(requestId: string, adminUserId: string, options: { rootFolderPath: string; qualityProfileId: number }) {
    ...
    try {
      ...
      await this.repository.complete(request.id, adminUserId, "approved", {
        providerItemId: result.title.id,
        rootFolderPath: result.rootFolderPath,
        qualityProfileId: result.qualityProfileId,
      });
