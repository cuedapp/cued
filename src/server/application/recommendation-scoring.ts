...
export function scoreCandidates(candidates: TmdbCandidate[], signals: RecommendationSignal[], previousScores = new Map<string, number>(), excludedTitles = new Set<string>(), similarSources = new Map<string, Array<{ id: number; type: TmdbMediaType; title: string; reason: "liked" | "watched" }>>()) {
  ...
  const strongestNegativeGenre = Math.max(1, ...[...taste.values()].map((genre) => Math.abs(Math.min(0, genre.weight))));
  ...
  return candidates
    ...
    .map((candidate): ScoredRecommendation => {
      ...
      const positiveAffinity = genreWeights.reduce((sum, genre) => sum + Math.max(0, genre.weight) / strongestPositiveGenre, 0) / genreCount;
      const negativeAffinity = genreWeights.reduce((sum, genre) => sum + Math.abs(Math.min(0, genre.weight)) / strongestNegativeGenre, 0) / genreCount;
      ...
      const matchPercent = Math.round(Math.max(1, Math.min(98, 40 + positiveAffinity * 30 - negativeAffinity * 15 + (sourceTitles.length > 0 ? 15 : 0) + candidate.rating + confidenceScore * 0.5)));
