...
function NotificationRow({ notification, locale, activeJobLabels }: { notification: Notification; locale: string; activeJobLabels: Set<string> }) {
  ...
  const message = notification.category.startsWith("request.") ? t(`events.${notification.category}.message`, { title: notification.message }) : notification.category === "jellyfin.completed" && !counts ? t("events.jellyfin.completed.messageFallback") : t(`events.${notification.category}.message`, counts);
