  ...
  return (
    <form action={action} onReset={(event) => event.preventDefault()} className="space-y-5">
      ...
      <div className="space-y-2">
        ...
        />
        <p className="text-xs text-muted-foreground">{t("baseUrlHelp")}</p>
      </div>
      <div className="space-y-2">
        ...
        <p className="text-xs text-muted-foreground">
          {overview.encryptionConfigured ? t("apiKeyHelp") : t("encryptionHelp")}
        </p>
      </div>
      {visibleRootFolders.length > 0 && (
        <div className="grid gap-5 sm:grid-cols-2">
          <div className="space-y-2">
            <Label htmlFor={`${provider}-root`}>{t("rootFolder")}</Label>
            <select
              id={`${provider}-root`}
              name="rootFolderPath"
          ...
          <div className="space-y-2">
            <Label htmlFor={`${provider}-profile`}>{t("qualityProfile")}</Label>
            <select
              id={`${provider}-profile`}
              ...
              className="h-10 w-full cursor-pointer rounded-lg border border-input bg-background px-3"
            >
              {visibleQualityProfiles.map((profile) => (
        ...
        <>
          <label className="flex cursor-pointer items-start gap-3 rounded-xl border border-border p-4">
            <input
              type="checkbox"
              name="searchOnAdd"
              defaultChecked={overview.searchOnAdd}
              className="mt-1 size-4 cursor-pointer accent-primary"
            />
            <span>
              <span className="block text-sm font-medium">{t("searchOnAdd")}</span>
              <span className="text-xs text-muted-foreground">{t("searchOnAddHelp")}</span>
            </span>
          </label>
          {provider === "sonarr" && (
            <div className="space-y-2">
              <Label htmlFor="seriesMonitor">{t("seriesMonitor")}</Label>
              <select
                id="seriesMonitor"
                name="seriesMonitor"
          ...
          {provider === "radarr" && <input type="hidden" name="seriesMonitor" value="all" />}
                ...
                {visibleTags.map((tag) => (
                  ...
                  >
                    <input
                      ...
                      name="tagIds"
                      value={tag.id}
                      defaultChecked={overview.tagIds.includes(tag.id)}
