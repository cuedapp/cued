      ...
      {users.map((user) => <Card key={user.id}>
        ...
        <CardContent>
          ...
          <div className="space-y-2">
            <RequestPolicyForm userId={user.id} locale={locale} requireApproval={user.requestsRequireApproval} disabled={user.role === "admin"} />
            <Link href={`/profile/${user.id}` as never} className="flex h-10 items-center justify-center rounded-lg border border-border px-3 text-sm font-medium hover:bg-accent">{t("viewProfile")}</Link>
