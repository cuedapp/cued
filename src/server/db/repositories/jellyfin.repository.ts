...
export class JellyfinRepository {
  ...
  async recordSuccessfulCheck(integrationId: string, info: { id: string; name: string; version: string }) {
    const now = new Date();
    await db
      .update(integrations)
      .set({
  ...
  async syncLibraries(integrationId: string, libraries: MediaLibrary[]) {
    ...
    if (libraries.length === 0) {
      ...
      await db.delete(mediaLibraries).where(
        and(
          eq(mediaLibraries.integrationId, integrationId),
          notInArray(
            mediaLibraries.jellyfinLibraryId,
  ...
  async getLibraries(integrationId: string) {
    return db.query.mediaLibraries.findMany({
  ...
  async setSelectedLibraries(integrationId: string, selectedIds: string[]) {
    ...
    for (const library of libraries) {
      await db
        ...
        .set({ selected: selectedIds.includes(library.jellyfinLibraryId), updatedAt: new Date() })
        .where(and(eq(mediaLibraries.id, library.id), eq(mediaLibraries.integrationId, integrationId)));
    }
  }
}
