...
describe("JellyfinIntegrationService", () => {
  it("allows URL setup before an API key is available", async () => {
    const repository = {
      getIntegration: vi.fn().mockResolvedValueOnce(undefined).mockResolvedValueOnce({ id: "integration", baseUrl: "http://jellyfin:8096", encryptedApiKey: null, status: "healthy" }),
      saveIntegration: vi.fn().mockResolvedValue({ id: "integration" }),
      getLibraries: vi.fn().mockResolvedValue([]),
      syncLibraries: vi.fn(),
    } as unknown as JellyfinRepository;
    const provider = { testConnection: vi.fn().mockResolvedValue({ id: "server", name: "Home", version: "10.10.0" }) } as unknown as MediaServerProvider;
    const overview = await new JellyfinIntegrationService(repository, undefined, () => provider).configure({ baseUrl: "http://jellyfin:8096/" });
    expect(repository.saveIntegration).toHaveBeenCalledWith(expect.objectContaining({ baseUrl: "http://jellyfin:8096", serverId: "server" }));
  ...
  it("encrypts an API key and discovers libraries", async () => {
    const repository = {
      getIntegration: vi.fn().mockResolvedValueOnce(undefined).mockResolvedValueOnce({ id: "integration", baseUrl: "http://jellyfin:8096", encryptedApiKey: "stored", status: "healthy" }),
      saveIntegration: vi.fn().mockResolvedValue({ id: "integration" }),
      getLibraries: vi.fn().mockResolvedValue([]),
      syncLibraries: vi.fn(),
    } as unknown as JellyfinRepository;
    ...
    await new JellyfinIntegrationService(repository, encryption, () => provider).configure({ baseUrl: "http://jellyfin:8096", apiKey: "api-key" });
    ...
    expect(repository.syncLibraries).toHaveBeenCalledWith("integration", [{ id: "movies", name: "Movies", collectionType: "movies" }]);
  });
});
