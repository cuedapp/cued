...
export function HistoryFilters({ query }: { query: HistoryQuery }) {
  ...
  return (
    <form
      onSubmit={onSubmit}
      className="grid gap-4 sm:grid-cols-2 lg:grid-cols-[repeat(4,minmax(0,1fr))_auto] lg:items-end"
    >
      <FilterSelect
        name="status"
        label={t("progressLabel")}
        value={query.status}
        options={(["completed", "inProgress", "all"] as const).map((value) => ({
          value,
      ...
      <button
        type="submit"
        disabled={isPending}
        className="h-10 cursor-pointer rounded-lg bg-primary px-5 text-sm font-medium text-primary-foreground hover:bg-primary/90 disabled:cursor-wait disabled:opacity-60 sm:col-span-2 lg:col-span-1"
      >
