...
describe("LibraryService", () => {
  it("keeps removed titles in the catalog and maps their discovery fields", async () => {
    ...
    const repository = { list: vi.fn().mockResolvedValue({ total: 1, items: [{
      ...
      raw: { Overview: "Still worth discovering", Genres: ["Mystery", "Drama"], CommunityRating: 7.8 }, removedAt,
      selectedRating: { source: "imdb", value: 7.9, scale: 10, normalizedScore: 7.9, votes: 1200 },
    }] }) } as unknown as LibraryRepository;

    const filters = { type: "all", state: "removed", query: "archived", genre: "Mystery", minimumRating: 7, ratingSource: "imdb", sort: "rating", intentPresets: [], intentText: "" } as const;
  ...
  it("clamps pages beyond the final catalog page", async () => {
    ...
    const filters = { type: "movie", state: "active", query: "", genre: "", minimumRating: null, ratingSource: "jellyfin", sort: "title", intentPresets: [], intentText: "" } as const;
