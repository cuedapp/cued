...
describe("AcquisitionService", () => {
  it("paginates a user's durable request history", async () => {
    const repository = {
      getForUser: vi
        ...
        .mockResolvedValueOnce({ total: 21, items: [{ request: { id: "last" }, reviewerName: null }] }),
  ...
  it("queues regular-user requests when approval is required", async () => {
    const repository = {
      findPending: vi.fn().mockResolvedValue(undefined),
      createPending: vi.fn().mockResolvedValue({ request: { id: "request" }, created: true }),
    } as unknown as AcquisitionRepository;
    const radarr = {
      getRequestState: vi.fn().mockResolvedValue("requestable"),
      request: vi.fn(),
    } as unknown as ArrIntegrationService;
    const result = await new AcquisitionService(repository, radarr, {} as ArrIntegrationService).request(
      regularUser,
      "movie",
      10,
      { rootFolderPath: "/not-allowed", qualityProfileId: 99 },
    );
  ...
  it("allows an individually exempted user to request directly", async () => {
    ...
    const radarr = {
      request: vi
        .fn()
        .mockResolvedValue({ state: "requested", title: { id: 7 }, rootFolderPath: "/movies", qualityProfileId: 2 }),
    } as unknown as ArrIntegrationService;
    const result = await new AcquisitionService(repository, radarr, {} as ArrIntegrationService).request(
      { ...regularUser, requestsRequireApproval: false },
      "movie",
      10,
    );
    ...
    expect(repository.complete).toHaveBeenCalledWith("request", "user", "approved", {
      providerItemId: 7,
  ...
  it("does not submit a title that already has a pending request", async () => {
    const repository = {
      findPending: vi.fn().mockResolvedValue({ id: "existing-request" }),
    } as unknown as AcquisitionRepository;
  ...
  it("submits an approved pending request and records its reviewer", async () => {
    const repository = {
      getById: vi.fn().mockResolvedValue({ id: "request", status: "pending", mediaType: "series", tmdbId: 20 }),
      complete: vi.fn(),
    } as unknown as AcquisitionRepository;
    const options = { rootFolderPath: "/series", qualityProfileId: 3 };
    const sonarr = {
      request: vi.fn().mockResolvedValue({ state: "requested", title: { id: 8 }, ...options }),
    } as unknown as ArrIntegrationService;
