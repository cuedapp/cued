...
describe("FollowService", () => {
  ...
  it("detects new seasons and requestability transitions", async () => {
    ...
    const repository = {
      list: vi.fn().mockResolvedValue([follow]),
      addEvent: vi.fn(),
      update: vi.fn(),
    } as unknown as FollowRepository;
    const title = { id: 10, title: "Series", seasons: 3, date: "2027-01-01", posterPath: "/poster.jpg" };
    const metadata = {
      refreshTitleMetadata: vi.fn().mockResolvedValue(title),
      getTitle: vi.fn().mockResolvedValue({ ...title, available: false }),
    } as unknown as TmdbMetadataService;
    const acquisition = { getState: vi.fn().mockResolvedValue("requestable") } as unknown as AcquisitionService;
    ...
    expect(repository.update).toHaveBeenCalledWith(
      "follow",
      expect.objectContaining({ snapshot: { seasonCount: 3 }, requestState: "requestable" }),
    );
  });
  ...
  it("detects credits added after a person was followed", async () => {
    ...
    const metadata = {
      getPersonMetadata: vi.fn().mockResolvedValue({
        id: 7,
        name: "Person",
        credits: [
          { id: 1, type: "movie", title: "Old", role: "Actor" },
          { id: 2, type: "series", title: "New", role: "Actor" },
        ],
      }),
    } as unknown as TmdbMetadataService;
    ...
    expect(repository.addEvent).toHaveBeenCalledWith(
      expect.objectContaining({ eventType: "new_credit", relatedTmdbId: 2, relatedTitle: "New" }),
    );
  });
});
