...
describe("ActivityService", () => {
  it("assembles private activity with visible server aggregates and fills missing trend days", async () => {
    const repository = {
      getRecentActivity: vi.fn().mockResolvedValue([
        {
          ...
          seasonNumber: null,
          episodeNumber: null,
          lastPlayedAt: new Date("2026-08-29T12:00:00Z"),
          playCount: 1,
        },
      ]),
      ...
      getTopRatedTitles: vi
        .fn()
        .mockResolvedValue([{ name: "Rated series", kind: "series", averageRating: "4.5", ratings: "2" }]),
    ...
    const activity = await new ActivityService(repository).getDashboardActivity(
      "user",
  ...
  it("normalizes aggregate timestamp values for the admin statistics page", async () => {
    ...
    expect(summaries[0]?.recent).toEqual([
      {
        name: "Recent film",
        kind: "movie",
        seriesName: null,
        seasonNumber: null,
        episodeNumber: null,
        lastPlayedAt: new Date("2026-08-29T12:00:00.000Z"),
      },
    ]);
  });
  ...
  it("normalizes the server and private profile statistics", async () => {
    ...
    await expect(service.getServerStatistics()).resolves.toMatchObject({
      movies: 12,
      series: 4,
      users: 3,
      watchedTitles: 18,
      estimatedWatchSeconds: 7200,
      ratings: 9,
      averageRating: 4.2,
      lastPlayedAt: new Date("2026-08-29T12:00:00.000Z"),
