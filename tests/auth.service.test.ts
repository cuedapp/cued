...
describe("AuthService", () => {
  it("maps an authenticated Jellyfin user and encrypts the access token", async () => {
    ...
    const result = await new AuthService(authRepository, jellyfinRepository, encryption, () => provider).login(
      "Erik",
      "password",
    );
    ...
    expect(authRepository.upsertUser).toHaveBeenCalledWith(
      "integration",
      expect.objectContaining({ id: "jellyfin-user", isAdministrator: true }),
    );
  ...
  it("rejects authentication from a different Jellyfin server", async () => {
    ...
    const jellyfinRepository = {
      getIntegration: vi
        .fn()
        .mockResolvedValue({ id: "integration", baseUrl: "http://jellyfin:8096", serverId: "expected" }),
    } as unknown as JellyfinRepository;
    const provider = {
      authenticate: vi.fn().mockResolvedValue({
        serverId: "other",
