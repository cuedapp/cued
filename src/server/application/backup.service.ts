...
export class BackupService {
  async exportUser(userId: string): Promise<UserExport> {
    ...
    return {
      ...
      tasteProfile: taste
        ? {
            onboardingStatus: taste.onboardingStatus,
            sourceMediaCount: taste.sourceMediaCount,
            profile: json(taste.profile),
            generatedAt: date(taste.generatedAt),
            completedAt: date(taste.completedAt),
          }
        : null,
      feedback: feedback.flatMap((entry) =>
        entry.tmdbId !== null &&
        (entry.mediaType === "movie" || entry.mediaType === "series" || entry.mediaType === "season")
          ? [
              {
      ...
      follows: userFollows.map((follow) => ({
        targetType: follow.targetType as "movie" | "series" | "person",
        tmdbId: follow.tmdbId,
        locale: follow.locale,
        title: follow.title,
        imagePath: follow.imagePath,
        releaseDate: follow.releaseDate,
        snapshot: json(follow.snapshot),
        requestState: follow.requestState,
  ...
  async importUser(userId: string, input: unknown) {
    ...
    await db.transaction(async (tx) => {
      await tx
        .update(users)
        .set({ ...archive.preferences, updatedAt: new Date() })
        .where(eq(users.id, userId));
      if (archive.tasteProfile)
        await tx
          ...
          .values({
            userId,
            onboardingStatus: archive.tasteProfile.onboardingStatus,
            sourceMediaCount: archive.tasteProfile.sourceMediaCount,
            profile: archive.tasteProfile.profile,
            generatedAt: archive.tasteProfile.generatedAt ? new Date(archive.tasteProfile.generatedAt) : null,
            completedAt: archive.tasteProfile.completedAt ? new Date(archive.tasteProfile.completedAt) : null,
            updatedAt: new Date(),
          })
          .onConflictDoUpdate({
      ...
      for (const item of archive.feedback) {
        const [media] = await tx
          ...
          .from(mediaItems)
          .where(and(eq(mediaItems.kind, item.mediaType), eq(mediaItems.tmdbId, item.tmdbId)));
        if (!media) {
          skippedFeedback++;
          continue;
        }
      ...
      for (const follow of archive.follows)
        await tx
          .insert(follows)
          .values({
            userId,
          ...
          .onConflictDoUpdate({
            target: [follows.userId, follows.targetType, follows.tmdbId],
            set: {
              locale: follow.locale,
              title: follow.title,
              imagePath: follow.imagePath,
  ...
  async exportFull() {
    const [
      ...
      db.select().from(mediaCollectionItems),
      db.select().from(userMediaFeedback),
      db.select().from(userTasteProfiles),
      db.select().from(recommendations),
      db.select().from(recommendationRefreshStates),
      db.select().from(userMediaStates),
      db.select().from(integrationSyncRuns),
      db.select().from(acquisitionRequests),
    ...
    return {
      ...
      data: json({
        ...
        userMediaFeedback: feedbackData,
        userTasteProfiles: tasteData,
        recommendations: recommendationsData,
        recommendationRefreshStates: refreshStatesData,
        userMediaStates: mediaStatesData,
        integrationSyncRuns: syncRunsData,
        acquisitionRequests: requestsData,
        follows: followsData,
  ...
  async restoreFull(input: unknown) {
    const parsed = z
      .object({
        format: z.literal("cued-full-backup"),
    ...
    await db.transaction(async (tx) => {
      ...
      for (const batch of batches("mediaCollectionItems")) await tx.insert(mediaCollectionItems).values(batch as never);
      ...
      for (const batch of batches("recommendationRefreshStates"))
        await tx.insert(recommendationRefreshStates).values(batch as never);
