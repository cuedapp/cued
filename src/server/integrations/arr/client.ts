...
const rootFoldersSchema = z.array(
  z.object({ id: z.number().int(), path: z.string(), freeSpace: z.number().optional() }).loose(),
);
...
export class ArrClient implements ArrProvider {
  ...
  async getStatus(connection: ArrConnection): Promise<ArrSystemStatus> {
    ...
    return {
      instanceName: value.instanceName || (this.kind === "radarr" ? "Radarr" : "Sonarr"),
  ...
  async getTags(connection: ArrConnection) {
    return tagsSchema.parse(await this.request(connection, "/tag"));
  }
    ...
    const payload =
      this.kind === "radarr"
        ...
        : {
            ...
            seasonFolder: true,
            addOptions: {
              monitor: options.seriesMonitor ?? "all",
  ...
  private async request(connection: ArrConnection, path: string, init: RequestInit = {}) {
    ...
    try {
      response = await this.transport(`${baseUrl}/api/v3${path}`, {
        ...init,
        headers: { Accept: "application/json", "Content-Type": "application/json", "X-Api-Key": connection.apiKey },
    ...
    if (!response.ok) {
      const body = (await response.json().catch(() => undefined)) as
        { message?: string; errorMessage?: string } | undefined;
      throw new ArrRequestError(
        response.status,
        body?.message || body?.errorMessage || `${this.kind === "radarr" ? "Radarr" : "Sonarr"} request failed`,
      );
    }
